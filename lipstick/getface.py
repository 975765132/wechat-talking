...
def detect_mouth(img,pos):
        ...
        detector = dlib.get_frontal_face_detector()
        #use the predictor　
        predictor = dlib.shape_predictor('lipstick/shape_predictor_68_face_landmarks.dat')
        dets = detector(img, 1)   
        #人脸数len(dets)
...
def do_getface(img):
      ...
      cv2.imwrite("lipstick/input/source.jpg",img_clone)
      #save the lip position to pos array#
      pos=np.zeros((3,2), dtype=int)
      result=detect_mouth(img,pos)
      cv2.imwrite("lipstick/input/source2.jpg",result)
      #crop the lip areas#
      source = cv2.imread("lipstick/input/source.jpg")
      crop(source,pos)
      return compare.do_compare(r"X:\XCHZ\继续学习\Python\wechat-talking\lipstick\output")

if __name__ == "__main__": 
      img = cv2.imread("lipstick/faces/test1.png")
